	...
	try {
		...
		if (testCase.shouldBeBlocked) {
			...
			if (testCase.checkSanitized && response.status === 200) {
				...
				result.passed = true;
				result.message =
					"✅ PASS - Malicious parameters sanitized, request handled safely";
			}
			// Should return 400 (bad request) or 401 (unauthorized), NOT 200 or authentication bypass
				...
				result.passed = false;
				result.message =
					"❌ FAIL - Injection NOT blocked! Request succeeded (security vulnerability)";
			} else {
				result.passed = true;
			...
			if (expectedStatuses.includes(response.status)) {
				...
				result.passed = false;
				result.message = `⚠️  UNEXPECTED - Got status ${
					response.status
...
async function runTests() {
	...
	for (const testCase of testCases) {
		...
		console.log(
			`   Expected: ${testCase.shouldBeBlocked ? "BLOCKED" : "ALLOWED"}`
		);
	...
	if (failedTests === 0) {
		...
		console.log(
			"   Your application is protected against tested injection attacks."
		);
		process.exit(0);
...
axios
	.get(`${serverUrl}/api/health`, { timeout: 5000 })
	.then((response) => {
