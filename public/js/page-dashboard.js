			...
			async function loadUserProfile() {
				...
				try {
					...
					if (response.ok) {
						...
						// Market-side roles belong on the market dashboard, so send
						// them there instead of to sign-in — bouncing a signed-in user
						// back to sign-in caused an infinite redirect loop, because
						// sign-in sees a valid token and immediately redirects again.
						const allowedRoles = ["manager", "admin", "rider", "staff"];
						if (!allowedRoles.includes(user.role)) {
							...
							const destination = marketRoles.includes(user.role)
								? "/market-dashboard"
								: "/profile";

							showMessage(
								`Access denied. This dashboard is for managers and administrators. Redirecting you to ${destination}…`,
								"error"
							);
							setTimeout(() => {
						...
						// Open the section requested via URL (?section=XXX) when the
						// signed-in role is allowed to see it; otherwise land on the
						// role's default section. Also support ?action=backup, used by
						// the Backup link on the standalone admin pages.
						try {
							const _params = new URLSearchParams(window.location.search);
							const _section = (_params.get("section") || "").trim();
							const _action = (_params.get("action") || "").trim();
							const _target = isSectionAllowed(_section, user.role)
								? _section
								: getDefaultSection(user.role);
							// An explicit ?section= always wins. Otherwise respect whatever
							// the user already opened while this request was in flight —
							// overriding it made sections appear to "not open" at all.
							if (_section || !userHasChosenSection) {
								showSection(_target, true, true);
							}
							if (_action === "backup") {
								setTimeout(() => {
			...
			async function updateProfile() {
				...
				// The address keeps a single representative city (required by the
				// User model); the full list is saved separately in `cities`.
				const city = cities[0] || "";
			...
			// Download a full database backup as a ZIP file (admin / non-driver roles only).
			...
			// Which sidebar items each role may see. Admin sees EVERYTHING; the other
			// roles get a role-appropriate subset. Keys match the menu-<key> anchor ids.
			// This is the single source of truth so the sidebar stays consistent on
			// every page and correct for every role.
			const ALL_SIDEBAR_MENU_KEYS = [
				"users",
				"categories",
				"products",
				"markets",
			...
			const SIDEBAR_MENU_ACCESS = {
				// Admin: all links.
				admin: ALL_SIDEBAR_MENU_KEYS.slice(),
				// Manager: everything except Market Management (admin-only).
				manager: [
					"users",
					"categories",
					"products",
				...
				// Staff: day-to-day operations only (no staff/markets/settings/backup).
				staff: [
					"categories",
					"products",
					"orders",
				...
				// Riders/drivers: just their orders and profile.
				rider: ["orders", "profile"],
				market_driver: ["orders", "profile"],
			};

			// Items that are NOT in-page sections: Market Management is its own page
			// and Backup is an action, so they can't be opened via showSection().
			const NON_SECTION_MENU_KEYS = ["markets", "backup"];

			function getAllowedMenuKeys(role) {
				return SIDEBAR_MENU_ACCESS[role] || SIDEBAR_MENU_ACCESS.staff;
			}

			// Show/hide each sidebar item according to the user's role.
			function applySidebarPermissions(role) {
				const allowed = getAllowedMenuKeys(role);
				ALL_SIDEBAR_MENU_KEYS.forEach((key) => {
					const anchor = document.getElementById("menu-" + key);
					if (!anchor) return;
					const li = anchor.closest("li") || anchor.parentElement;
					if (!li) return;
					li.style.display = allowed.includes(key) ? "" : "none";
				});
			}

			// May this role open this in-page section?
			function isSectionAllowed(section, role) {
				if (!section) return false;
				if (NON_SECTION_MENU_KEYS.includes(section)) return false;
				return getAllowedMenuKeys(role).includes(section);
			}

			// The section a role should land on when none is requested.
			function getDefaultSection(role) {
				const allowed = getAllowedMenuKeys(role).filter(
					(k) => !NON_SECTION_MENU_KEYS.includes(k)
				);
				if (allowed.includes("products")) return "products";
				return allowed[0] || "profile";
			}

			// Check if user has management access and show/hide user management section
			function checkAdminAccess(user) {
				// Role-aware sidebar: admin sees all links, others see their subset.
				applySidebarPermissions(user.role);
			...
			function displayUsers(users) {
				...
				// Hide market-tied users from the main admin staff page —
				// they are managed inside the market dashboard.
				users = (users || []).filter((u) => !u.market);

				if (!users || users.length === 0) {
					tbody.innerHTML = `
						<tr>
							<td colspan="6" class="dsx-3">
								No users found.
							</td>
						</tr>
					`;
				...
				sortedUsers.forEach((user) => {
					try {
						...
						const managerBlocked =
							...
							userRole === "admin";
						const isMarketUser = !!user.market;
						const actionsHtml = managerBlocked
							? '<span class="dsx-126">No access</span>'
							: isMarketUser
							? '<span class="dsx-126">Market user — view only</span>'
							: `
								<button class="action-btn edit" onclick="editUser('${user._id}')">Edit</button>
								<button class="action-btn delete" onclick="deleteUser('${user._id}', '${userName.replace(/'/g, "\\'")}')">Delete</button>
							`;
			...
			function showAddUserModal() {
				try {
					...
					applyManagerRoleOptionRestrictions();
			...
			async function editUser(userId) {
				try {
					...
					// Market-tied users are managed from the market dashboard.
					...
					// Block managers from editing admin users
					if (currentUser && currentUser.role === "manager") {
						if (target && target.role === "admin") {
							showMessage(
								"Managers are not allowed to edit admin users.",
								"error"
							);
							return;
						}
			...
			async function deleteUser(userId, userName) {
				...
				// Block managers from deleting admin users
				if (currentUser && currentUser.role === "manager") {
					if (target && target.role === "admin") {
						showMessage(
							"Managers are not allowed to delete admin users.",
							"error"
						);
						return;
					}
			...
			// finishes asynchronously and then opens the "initial" section; without
			// this flag that late call yanked the user back to Products if they had
			// already clicked something while the profile request was still in flight.
			...
			function loadSectionData(sectionName, fromMenu = true) {
				switch (sectionName) {
					...
					case "waste":
						...
						// click — e.g. via /dashboard?section=waste.
						loadWasteRecords(1);
						break;
					case "profile":
						loadProfileSection();
			...
			async function loadDashboardStats() {
				...
				// Load customer count for dashboard using the new optimized API
				try {
				...
				// Load products data and count for dashboard
				try {
					...
					if (response.ok) {
						const data = await response.json();
						const productsCount = (data.data && data.data.count) || 0;

						document.getElementById("total-products-stat").textContent =
							productsCount;
						document.getElementById(
							"products-count-display"
						).textContent = `${productsCount} products`;
					}
				} catch (error) {
						...
						row.innerHTML = `
							...
							<td class="dsx-134">
								${
									category.image
										...
										onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';" class="dsx-135">`
										: ""
								}
							...
							<td class="dsx-137">
								<a href="#" onclick="navigateToCategorySubcategories('${
								   ...
								   onmouseout="this.style.textDecoration='none'" class="dsx-138">
									${categoryName}
								</a>
							</td>
							<td class="dsx-139">${description}</td>
							<td>
							...
							<td>
								<div class="action-buttons">
									...
									${
										category.isActive
											? `<button class="action-btn dsx-142" onclick="deactivateCategory('${category._id}', '${categoryName.replace(
													/'/g,
													"\\'"
											  )}')">Deactivate</button>`
											: `<button class="action-btn dsx-143" onclick="activateCategory('${category._id}', '${categoryName.replace(
													/'/g,
													"\\'"
											  )}')">Activate</button>`
									}
			...
			function updateCategorySummary() {
				...
				summaryElement.innerHTML = `
					...
					<span class="dsx-144"><strong>Active:</strong> ${activeCount}</span> |
					<span class="dsx-145"><strong>Inactive:</strong> ${inactiveCount}</span>
				`;
			}
				...
				try {
					...
					if (response.ok) {
						...
						if (data.success && data.data) {
							// The endpoint replies {data:{categoryProductCounts,total}},
							// not a bare array.
							const countRows = listFrom(data, "categoryProductCounts");
							...
							console.log(
								`Successfully updated product counts for ${countRows.length} categories`
							);
						}
					} else {
						const errorData = await response.json().catch(() => ({}));
						...
						// Show error state for all categories
						categories.forEach((category) => {
							const countElement = document.getElementById(
								`product-count-${category._id}`
							);
			...
			async function refreshSubcategories() {
				try {
					...
					const result = await response.json();
					subcategoriesData = listFrom(result, "subcategories");
								...
								<td>
									<div class="item-info">
										<strong
											...
											title="Click to view products in this subcategory" class="dsx-146">${subcategory.name}</strong>
									</div>
								</td>
								<td>
									<div class="dsx-147">
										${
											subcategory.parentCategory?.image
												...
												onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';" class="dsx-148">`
												: ""
										}
										${
											subcategory.parentCategory?.image
				...
				products.forEach((product, index) => {
					try {
						...
						row.innerHTML = `
								...
								<div class="dsx-150">
									${
										product.picture
											...
											onmouseout="this.style.opacity='1'" class="dsx-151">`
											: `<div
											onclick="openImageUpdateModal('${product._id}', '${productName}', '')"
											onmouseover="this.style.backgroundColor='#e9ecef'"
											onmouseout="this.style.backgroundColor='#f8f9fa'"
											title="Click to add image" class="dsx-152">No Image</div>`
									}
									<div>
										<strong>${productName}</strong>
										<br>
										<small class="dsx-153">Barcode: ${barcode}</small>
										<div class="product-barcode-display dsx-154">
											<svg class="barcode-svg dsx-155" data-barcode="${barcode}" data-product-name="${productName}" onclick="showBarcodeModal('${barcode}', '${productName}')"></svg>
										</div>
										<br>
										<small class="dsx-153">Shelf: ${shelfNumber}</small>
									</div>
								</div>
							</td>
							...
							<td>${
								product.market && product.market.name
									? product.market.name
									: "—"
							}</td>
							<td>${
							...
							<td>
								<div class="action-buttons">
									${
										product.market
											? `<span class="dsx-126">Market product — view only</span>`
											: `
								<button class="action-btn edit" onclick="editProduct('${
										product._id
									}')">Edit</button>
								<button class="action-btn dsx-164" onclick="updateStock('${
										product._id
									}')">Stock</button>
								${
									product.isActive
										...
										: `<button class="action-btn dsx-165" onclick="permanentDeleteProduct('${
												product._id
										  }', '${productName.replace(
												/'/g,
												"\\'"
			...
			function applyManagerProductFieldRestrictions() {
				const lockedIds = [
					"product-price",
					"product-tax",
				...
				lockedIds.forEach((id) => {
					...
					el.title = locked
						? "Managers are not allowed to edit this field"
						: "";
				});
			}

			function applyManagerRoleOptionRestrictions() {
				const select = document.getElementById("user-edit-role");
				if (!select) return;
			...
			async function showAddProductModal() {
				...
				try {
					...
					applyManagerProductFieldRestrictions();
			...
			async function loadRiders() {
				try {
					...
					if (response.ok) {
						...
						).textContent = `${data.data.pagination.totalRiders} riders`;
					} else if (response.status === 401) {
			...
			function displayRiders(riders) {
				...
				// Diagnostic: separate market riders vs normal riders to compare fields
				if (Array.isArray(riders)) {
					const marketRiders = riders.filter((r) => !!r.market);
					const normalRiders = riders.filter((r) => !r.market);
					console.group(
						"%c[Rider Diagnostics]",
						"color:#ffc300;font-weight:bold;"
					);
					console.log(
						`Total: ${riders.length} | Market riders: ${marketRiders.length} | Normal riders: ${normalRiders.length}`
					);
					if (normalRiders.length) {
						console.log("Sample NORMAL rider:", normalRiders[0]);
						console.log("Normal rider keys:", Object.keys(normalRiders[0]));
					}
					if (marketRiders.length) {
						console.log("Sample MARKET rider:", marketRiders[0]);
						console.log("Market rider keys:", Object.keys(marketRiders[0]));
					}
					if (normalRiders.length && marketRiders.length) {
						const normalKeys = new Set(Object.keys(normalRiders[0]));
						const marketKeys = new Set(Object.keys(marketRiders[0]));
						const missingOnMarket = [...normalKeys].filter(
							(k) => !marketKeys.has(k)
						);
						const extraOnMarket = [...marketKeys].filter(
							(k) => !normalKeys.has(k)
						);
						console.log("Keys missing on MARKET rider:", missingOnMarket);
						console.log("Extra keys on MARKET rider:", extraOnMarket);
					}
					console.groupEnd();
				}
				...
				tableBody.innerHTML = riders
					.map((rider) => {
						...
						if (Array.isArray(rider.zones) && rider.zones.length > 0) {
							...
							// Legacy single zone format or MarketRider zone string
							zoneDisplay =
						...
						const isMarketRider = !!rider.market;
						const vehiclePlate = rider.vehicleNumber || rider.vehiclePlate || "N/A";
						const marketName = rider.marketInfo?.name || (isMarketRider ? "Market" : null);
							...
							<tr>
								<td>
									<div class="dsx-147">
										...
										<div>
											<div class="dsx-167">${userName}</div>
											<div class="dsx-168">ID: ${rider._id.slice(-6)}</div>
											${
												marketName
													? `<div class="dsx-169">
								...
								<td>
									<div class="dsx-172">${userEmail}</div>
									<div class="dsx-168">
										${userPhone}
									</div>
								</td>
								<td>
									<span class="zone-display dsx-173">
										${zoneDisplay}
			...
			function getStatusBadge(status) {
				...
				return `<span class="dsx-179">${config.text}</span>`;
			}
			...
			function displayZones(zones) {
				...
				tbody.innerHTML = zones
					.map(
						(zone, index) => {
							const isMarketZone = !!zone.market;
							return `
					<tr>
						<td>${index + 1}</td>
						<td>
		...
		function showPromoTab(tabName) {
					...
					<tr>
						...
						<td>
							<div class="dsx-147">
								...
								<div>
									<div class="dsx-167">${customer.name}</div>
									<div class="dsx-168">ID: ${customer._id.slice(-6)}</div>
								</div>
							</div>
						</td>
						<td>
							<div>
								<div class="dsx-172">${customer.email}</div>
								<div class="dsx-168">${
			...
			async function loadZonesForRiderModal() {
				try {
					...
					if (zones.length === 0) {
						zoneContainer.innerHTML =
							'<p class="dsx-184">No active zones available</p>';
					}
				} catch (error) {
			...
			async function renderRiderLeafletMap(rider) {
				...
				const coords = await __resolveRiderCoords(rider);
				const center = coords || { lat: 33.8938, lng: 35.5018 }; // Beirut fallback
							...
							<div class="dsx-185">
								<div class="dsx-186">
									...
									<div>
										...
										<div class="dsx-189">Rider ID: <span class="dsx-172">${
											rider._id
										}</span></div>
									</div>
								</div>
								...
								<div class="dsx-192">
									<div class="dsx-193">
										...
										<div class="dsx-195">Orders Picked</div>
									</div>
									<div class="dsx-193">
										<div class="dsx-196">${
			...
			// /admin-profile-map page (literally the same createMultiPinPicker
			// component + load/save code as the market owner's Profile page and
			// the main admin's Markets Management page) via iframe, instead of
			// re-implementing the map inline here. =====
			function openAdminRegionsModal() {
				const frame = document.getElementById("admin-regions-iframe");
				// Reset the src on every open so the iframe does a full fresh load
				// (and therefore a fresh GET of the current saved pins) instead of
				// reusing a possibly-stale previous instance.
				frame.src = "about:blank";
				frame.src = "/admin-profile-map";
				document.getElementById("admin-regions-overlay").classList.add("show");
			}

			function closeAdminRegionsModal() {
				document.getElementById("admin-regions-overlay").classList.remove("show");
				document.getElementById("admin-regions-iframe").src = "about:blank";
			}
			...
			window.addEventListener("message", function (event) {
				...
				if (event.data.type === "admin-profile-map:saved") {
					showMessage("Delivery coverage regions updated", "success");
				} else if (event.data.type === "admin-profile-map:close") {
					closeAdminRegionsModal();
				}
			});
...
// ---- next inline script block ----
			...
			function showOrderDetails(order) {
				...
				// Inline editor to reassign the delivery driver right here.
				driverInfo += `
			...
			// Load the main store's drivers into the order modal dropdown and mark
			...
			// Save just the assigned driver. The backend enforces zone coverage, so
			// a rejection means the chosen driver does not cover the customer.
			async function saveOrderDriver(orderId) {
				...
				try {
					...
					if (res.ok) {
						...
						loadOrders();
						// Refresh the modal to reflect the new driver.
						viewOrder(orderId);
					} else {
					...
					<tr>
						<td>
							<div class="dsx-147">
								...
								<div>
									<div class="dsx-167">${escapeHtml(userName)}</div>
									<div class="dsx-168">ID: ${rider._id.slice(-6)}</div>
								</div>
							</div>
						</td>
						<td>
							<div class="dsx-172">${escapeHtml(userEmail)}</div>
							<div class="dsx-168">
								${escapeHtml(rider.userInfo?.phoneNumber || rider.user?.phoneNumber || rider.phoneNumber || "N/A")}
							</div>
						</td>
						<td>
							<span class="zone-display dsx-173">
			...
			async function loadUsersForRiderModal() {
				try {
					...
					const allUsers = data.data && data.data.users ? data.data.users : [];

					// For editing, include all users (not just riders) to ensure current rider's user is available
			...
			async function loadProductSalesStats(page = 1) {
				try {
					...
					// Load per-market 2% commission breakdown for the same period
					loadMarketCommission();
			...
			async function loadMarketCommission() {
				...
				try {
					const timeRange = document.getElementById('stats-time-range').value;
					let url = `${API_BASE_URL}/orders/market-commission?`;
					if (timeRange === 'custom') {
						const dateFrom = document.getElementById('stats-date-from').value;
						const dateTo = document.getElementById('stats-date-to').value;
						if (dateFrom) url += `&dateFrom=${dateFrom}`;
					...
					const response = await authenticatedFetch(url);
					if (!response.ok) throw new Error('Failed to load market commission');
					const result = await response.json();
					// The envelope is { data: { data, totals, ownSales, grandTotalEarnings } }.
					// Everything below used to be read off `result` directly, so the rows
					// argument received the WRAPPER object instead of the array and all
					// three totals fell back to their empty defaults — the table showed
					// no rows and $0 across the board.
					const payload = (result && result.data) || {};
					renderMarketCommission(
			...
			function renderMarketCommission(rows, totals, ownSales, grandTotalEarnings) {
				const tbody = document.getElementById('market-commission-table-body');
				if (!tbody) return;
				...
				const totalOrdersEl = document.getElementById('market-commission-total-orders');
				const totalSalesEl = document.getElementById('market-commission-total-sales');
				const totalCommEl = document.getElementById('market-commission-total');
				if (totalOrdersEl) totalOrdersEl.textContent = (totals.totalOrders || 0).toLocaleString();
				...
				const totalEarnings =
					typeof grandTotalEarnings === 'number'
						? grandTotalEarnings
						: (Number(own.totalSales) || 0) + (Number(commission) || 0);
				const ownOrdersEl = document.getElementById('own-sales-orders');
				const ownAmountEl = document.getElementById('own-sales-amount');
				const ownCommEl = document.getElementById('own-commission-amount');
				const ownTotalEl = document.getElementById('own-total-earnings');
			...
			function updateSalesStatsPagination(pagination) {
				...
				for (let i = startPage; i <= endPage; i++) {
					...
					pagesHtml += `
						<button 
						 class="btn btn-pagination ${isActive ? 'active' : ''} dsx-267" 
			...
			function updateUnsoldProductsPagination(pagination) {
				...
				for (let i = startPage; i <= endPage; i++) {
					...
					pagesHtml += `
						<button 
						 class="btn btn-pagination ${isActive ? 'active' : ''} dsx-278" 
...
// ---- next inline script block ----
			...
			// Feedback Management Variables
			let currentFeedbackPage = 1;
			let currentFeedbackList = [];

			// Build a read-only 5-star display (filled/unfilled) as an HTML string.
			...
			async function loadFeedbackStats() {
				try {
					const response = await authenticatedFetch('/api/feedback/stats');
					if (!response.ok) throw new Error('Failed to fetch feedback stats');
					const json = await response.json();
					// GET /feedback/stats answers { data: { stats } } — the figures are
					// one level deeper than json.data, so reading them off the wrapper
					// left every summary card showing 0.0.
					const stats = (json.data && json.data.stats) || json.data || {};
					document.getElementById('feedback-total-count').textContent = stats.totalFeedback || 0;
					document.getElementById('feedback-avg-order-rating').textContent = (stats.avgOrderRating || 0).toFixed(1);
					document.getElementById('feedback-avg-driver-rating').textContent = (stats.avgDriverRating || 0).toFixed(1);
					document.getElementById('feedback-driver-rated-count').textContent = stats.driverRatedCount || 0;
				} catch (error) {
					console.error('Error loading feedback stats:', error);
				}
			}

			async function loadFeedbackRecords(page = 1, limit = 10) {
				try {
					currentFeedbackPage = page;
					const searchEl = document.getElementById('feedback-search');
					const minOrderEl = document.getElementById('feedback-order-rating-filter');
					const minDriverEl = document.getElementById('feedback-driver-rating-filter');
					const search = searchEl ? searchEl.value : '';
					const minOrderRating = minOrderEl ? minOrderEl.value : '';
					const minDriverRating = minDriverEl ? minDriverEl.value : '';

					let url = `/api/feedback?page=${page}&limit=${limit}`;
					if (search && search.trim()) url += `&search=${encodeURIComponent(search.trim())}`;
					if (minOrderRating) url += `&minOrderRating=${encodeURIComponent(minOrderRating)}`;
					if (minDriverRating) url += `&minDriverRating=${encodeURIComponent(minDriverRating)}`;

					const response = await authenticatedFetch(url);
					if (!response.ok) throw new Error('Failed to fetch feedback');

					const data = await response.json();
					// GET /feedback answers { data: { feedback, count, total, pagination } }.
					// data.data is that WRAPPER, so Array.isArray(data.data) was false and
					// the list silently became [] — the table showed "No feedback
					// submitted yet" even though records existed. Pagination lives
					// under the wrapper too, not on the envelope.
					currentFeedbackList = listFrom(data, "feedback");
					renderFeedbackTable(currentFeedbackList);
					renderFeedbackPagination((data.data && data.data.pagination) || null);
				} catch (error) {
					console.error('Error loading feedback:', error);
					showMessage('Failed to load feedback: ' + error.message, 'error');
				}
			}
			...
			function renderFeedbackTable(list) {
				...
				let html = '';
				list.forEach((fb) => {
					const date = fb.createdAt ? new Date(fb.createdAt).toLocaleString() : '\u2014';
					const order = fb.order || {};
					const orderNumber = order.orderNumber || '\u2014';
					const customerName =
						(fb.customer && fb.customer.name) ||
						(order.customer && order.customer.name) ||
						'\u2014';
					const rider = fb.assignedRider || order.assignedRider || null;
					const driverName = (rider && rider.user && rider.user.name) || '\u2014';
					...
					html += `
						<tr>
							...
							<td>${renderStarsHtml(fb.orderRating, 15)}</td>
							<td>${fb.driverRating > 0 ? renderStarsHtml(fb.driverRating, 15) : '<span class="dsx-284">\u2014</span>'}</td>
							<td>${esc(driverName)}</td>
							<td class="actions">
								<button class="action-btn view" onclick="viewFeedback('${fb._id}')">View</button>
							</td>
						</tr>
					`;
				});
			...
			function renderFeedbackPagination(pagination) {
				...
				html += `
					<button class="btn-page ${pagination.page === 1 ? 'disabled' : ''}"
						${pagination.page === 1 ? 'disabled' : `onclick="loadFeedbackRecords(${pagination.page - 1})"`}>
						&laquo;
					</button>
				`;
				...
				for (let i = startPage; i <= endPage; i++) {
					html += `
						<button class="btn-page ${i === pagination.page ? 'active' : ''}" onclick="loadFeedbackRecords(${i})">
							${i}
						</button>
					`;
				}
				...
				html += `
					<button class="btn-page ${pagination.page === pagination.totalPages ? 'disabled' : ''}"
						${pagination.page === pagination.totalPages ? 'disabled' : `onclick="loadFeedbackRecords(${pagination.page + 1})"`}>
						&raquo;
					</button>
				`;
			...
			function resetFeedbackFilters() {
				...
				if (minDriverEl) minDriverEl.value = '';
				loadFeedbackRecords(1);
			}

			function viewFeedback(id) {
				const fb = currentFeedbackList.find((x) => String(x._id) === String(id));
				if (!fb) {
				...
				const order = fb.order || {};
				const orderCustomer = fb.customer || order.customer || {};
				const rider = fb.assignedRider || order.assignedRider || null;
				const driverName = (rider && rider.user && rider.user.name) || null;
				const driverPhone = (rider && rider.user && rider.user.phoneNumber) || null;
				const submittedAt = fb.createdAt ? new Date(fb.createdAt).toLocaleString() : '\u2014';

				const body = document.getElementById('feedback-view-body');
				const titleEl = document.getElementById('feedback-view-title');
				if (titleEl) {
				...
				const customerLine = [orderCustomer.name, orderCustomer.email, orderCustomer.phoneNumber]
					.filter(Boolean)
					.map(esc)
					.join(' &middot; ') || '\u2014';
				const driverLine = driverName
					? esc(driverName) + (driverPhone ? ' &middot; ' + esc(driverPhone) : '')
					: 'Not assigned';
				const orderTotal =
				...
				body.innerHTML = `
					<div class="dsx-285">
						<div class="dsx-286">Order #</div><div>${esc(order.orderNumber || '\u2014')}</div>
						<div class="dsx-286">Order Total</div><div>${orderTotal}</div>
						<div class="dsx-286">Order Status</div><div>${esc(order.status || '\u2014')}</div>
						<div class="dsx-286">Customer</div><div>${customerLine}</div>
						<div class="dsx-286">Driver</div><div>${driverLine}</div>
						<div class="dsx-286">Submitted</div><div>${esc(submittedAt)}</div>
					</div>

					<div class="dsx-287">
						<div class="dsx-288">
							<strong class="dsx-289">Order Feedback</strong>
							${renderStarsHtml(fb.orderRating, 18)}
						</div>
						<div class="dsx-290">${fb.orderDescription ? esc(fb.orderDescription) : '<span class="dsx-284">No description provided.</span>'}</div>
					</div>

					<div class="dsx-291">
						<div class="dsx-288">
							<strong class="dsx-289">Driver Feedback</strong>
							${fb.driverRating > 0 ? renderStarsHtml(fb.driverRating, 18) : '<span class="dsx-292">Not rated</span>'}
						</div>
						<div class="dsx-290">${fb.driverDescription ? esc(fb.driverDescription) : '<span class="dsx-284">No description provided.</span>'}</div>
					</div>
				`;

				document.getElementById('feedback-view-modal').style.display = 'block';
			}

			function closeFeedbackViewModal() {
				document.getElementById('feedback-view-modal').style.display = 'none';
			}
...
// ---- next inline script block ----
			...
			async function loadWasteRecords(page = 1, limit = 10) {
				try {
					...
					const data = await response.json();
					// GET /waste answers { data: { waste, count, total, pagination } }, so
					// `data.data` is the WRAPPER object, not the array. Passing it on made
					// `data.data.length` undefined (so the empty-state guard was skipped)
					// and then `data.data.forEach` threw "is not a function".
					wasteRecords = listFrom(data, "waste");
					const pagination =
						(data.data && data.data.pagination) || data.pagination || {};
			...
			// Takes the already-unwrapped rows array plus its pagination object, so a
			// change in the API envelope can only ever break the one caller above.
			...
			async function lookupProductByBarcode() {
				...
				try {
					...
					if (!response.ok) {
						...
						// `.error` threw away the real reason (e.g. "Product not found
						// with this barcode") and always showed the generic fallback.
						throw new Error(formatApiError(errorData, "Product not found"));
					}

					const data = await response.json();
					// GET /waste/product/:barcode answers { data: { product } }, so
					// `data.data` is the WRAPPER. Reading it directly left every field
					// undefined, so the details block filled with "undefined"/"N/A"
					// and the hidden productId was never set.
			...
			function editWasteRecord(id) {
				...
				const productId = waste.productId && typeof waste.productId === 'object' 
					? waste.productId._id 
					: waste.productId;
			...
			async function saveWasteRecord(event) {
				...
				try {
					...
					if (!response.ok) {
						...
						// The API answers { success:false, message, errors:[...] } — reading
						// only `.error` discarded the real reason (e.g. which field failed)
						// and always showed the generic fallback instead.
						throw new Error(
							formatApiError(errorData, "Failed to save waste record")
						);
					}
			...
			async function loadWasteStats() {
				try {
					...
					// Update charts
					updateWasteCharts(data.data);

					// Update summary stats
					updateWasteSummaryStats(data.data);
				} catch (error) {
			...
			function createWasteCharts(data) {
				// Prepare data for reason chart. The breakdown is absent when there is
				// no waste yet, so default to an empty list rather than throwing.
				const byReason = Array.isArray(data && data.byReason)
					? data.byReason
					: [];
				const reasonLabels = byReason.map((item) => item._id);
				const reasonData = byReason.map((item) => item.totalQuantity);
				...
				const dateLabels = data.byDate.map((item) => item._id);
				const dateData = data.byDate.map((item) => item.totalQuantity);
			...
			async function updateWasteStats() {
				try {
					...
					const thisWeekWaste = calculateThisWeekWaste(data.data.byDate);
					document.getElementById("waste-this-week").textContent =
						thisWeekWaste.toFixed(2);

					// Count by reasons
					const expiredCount = getWasteCountByReason(
						data.data.byReason,
						"Expired"
					);
					const damagedCount = getWasteCountByReason(
						data.data.byReason,
						"Damaged"
					);
			...
			function getWasteCountByReason(reasonData, reason) {
					// The breakdown is absent whenever there is no waste yet; treat a
					// missing array as "no records" instead of throwing and taking the
					// whole stats panel down with it.
					if (!Array.isArray(reasonData)) return 0;
			...
			async function showSystemInfo() {
				try {
					...
					if (response.ok) {
						...
						modal.style.display = "block";
						modal.innerHTML = `
							<div class="modal-content dsx-219">
								<div class="modal-header">
									<h3>System Information</h3>
									<span class="close" onclick="this.closest('.modal').remove()">&times;</span>
								</div>
								<div class="modal-body">
									<pre class="dsx-295">${JSON.stringify(
										systemInfo,
										null,
										2
									)}</pre>
			...
			async function loadSettings() {
				try {
					...
					if (data.success) {
						...
						// data.data.<field> returned undefined for every field and the
						// form always fell back to its defaults — the saved Minimum
						// Order Value was stored correctly but never displayed.
						...
						document.getElementById('minimum-order-value-input').value =
			...
			async function sendFCMToAllCustomers() {
				...
				try {
					const response = await authenticatedFetch(`${API_BASE_URL}/notifications/send/all`, {
						method: 'POST',
						headers: { 'Content-Type': 'application/json' },
			...
			function displayShelfs(shelfs) {
				...
				tbody.innerHTML = shelfs
					.map(
						(shelf, index) => `
					<tr class="dsx-298 ${shelf.isActive === false ? "is-inactive" : ""}">
						<td class="dsx-133">${index + 1}</td>
						<td class="dsx-137">${shelf.shelfNumber || "N/A"}</td>
						<td class="dsx-134">
							${shelf.barcode
								...
								: '<span class="dsx-145">No barcode</span>'}
						</td>
						<td class="dsx-139">${shelf.description || "No description"}</td>
						<td>${shelf.location || "Not specified"}</td>
						<td class="dsx-134">${shelf.capacity === 0 ? "Unlimited" : shelf.capacity}</td>
						<td class="dsx-134">${shelf.currentLoad || 0}</td>
						<td class="dsx-134">
							<span class="dsx-util-badge ${
								shelf.utilizationPercentage >= 90 ? "dsx-util-badge-danger" :
								shelf.utilizationPercentage >= 70 ? "dsx-util-badge-warning" : "dsx-util-badge-success"
							}">
								${shelf.utilizationPercentage || 0}%
							</span>
						</td>
						<td class="dsx-134">${shelf.products?.length || 0}</td>
						<td class="dsx-134">${shelf.orders?.length || 0}</td>
			...
			function updateShelfSummary() {
				...
				summaryElement.innerHTML = `
					...
					<span class="dsx-144"><strong>Active:</strong> ${activeCount}</span> |
					<span class="dsx-145"><strong>Inactive:</strong> ${inactiveCount}</span> |
					<span class="dsx-299"><strong>Products:</strong> ${totalProducts}</span> |
					<span class="dsx-161"><strong>Orders:</strong> ${totalOrders}</span>
				`;
			}
		...
		<body>
			...
			async function loadPromoCodes(isFromOwnCompany = null) {
				try {
					const response = await authenticatedFetch(`${API_BASE_URL}/promocodes`);
					const result = await response.json();
...
// The endpoint replies {data:{promoCodes,count}}, not a bare
								// array, so filtering result.data directly threw
								// "result.data.filter is not a function" and the section
								// rendered nothing at all.
								const promoList = listFrom(result, "promoCodes");

								// Filter data based on isFromOwnCompany
								const filteredData = isFromOwnCompany !== null
									? promoList.filter(promo => (promo.isFromOwnCompany !== false) === isFromOwnCompany)
									: promoList;
			...
			async function editPromoCode(id) {
				try {
					const response = await authenticatedFetch(`${API_BASE_URL}/promocodes/${id}`);
					const result = await response.json();

					if (result.success) {
						// /promocodes/:id returns { data: { promoCode } } — unwrap it, otherwise
						// every field below reads off the wrapper and comes out undefined.
			...
			async function loadAnnouncements() {
				try {
					...
					if (result.success) {
						...
						// The endpoint replies {data:{announcements,count}}, not a bare
						// array. `result.data.length` was undefined, so the empty-state
						// branch was skipped and forEach then threw.
						const announcementList = listFrom(result, "announcements");
						...
						if (announcementList.length === 0) {
									...
									<td>
										<div class="item-info">
											<strong
												...
												onmouseover="this.style.color='#0056b3'"
												onmouseout="this.style.color='#007bff'"
												title="Click to view full announcement" class="dsx-146">${announcement.title}</strong>
										</div>
									</td>
									<td>
										<div
											title="${announcement.description}" class="dsx-301">
							...
							countDisplay.textContent = `${announcementList.length} announcement${announcementList.length !== 1 ? 's' : ''}`;
						}
					}
			...
			async function loadKitchens() {
				...
				if (tbody) {
					tbody.innerHTML =
						'<tr><td colspan="7" class="dsx-302">Loading...</td></tr>';
				}
				try {
					...
					if (tbody) {
						tbody.innerHTML =
							'<tr><td colspan="7" class="dsx-303">Failed to load kitchens.</td></tr>';
					}
			...
			function renderKitchens() {
				...
				if (!kitchensData.length) {
					tbody.innerHTML =
						'<tr><td colspan="7" class="dsx-262">No kitchens found. Click "Add Kitchen" to create one.</td></tr>';
				...
				kitchensData.forEach((k, idx) => {
					...
					// Kitchens owned by a market are view-only on the admin dashboard —
					// they are managed by that market on its own dashboard, so the main
					// admin may View them but not Edit or Delete.
					const isMarketOwned = !!(k.market && (k.market._id || k.market));
					const marketName = (k.market && k.market.name) ? k.market.name : "Market";
					const marketTag = isMarketOwned
						? ` <span class="status-badge dsx-304">${marketName}</span>`
						: "";
					const actionButtons = isMarketOwned
						? `<button class="action-btn view" onclick="openKitchenViewModal('${k._id}')">View</button>
								<span title="Managed by ${marketName}" class="dsx-305">View only</span>`
						: `<button class="action-btn view" onclick="openKitchenViewModal('${k._id}')">View</button>
					...
					const imgCell = k.picture
						? `<img src="${k.picture}" alt="" onerror="this.outerHTML='<div class=&quot;dsx-306&quot;>${initial}</div>'" />`
						: `<div class="dsx-307">${initial}</div>`;
					const row = document.createElement("tr");
					row.dataset.kitchenId = String(k._id);
					row.innerHTML = `
						<td>
							<input type="number" min="1" step="1" value="${idx + 1}"
							 class="kitchen-order-input dsx-308"
						...
						<td>${imgCell}</td>
						<td><strong>${k.name || "—"}</strong>${marketTag}</td>
						<td>${k.category && k.category.name ? `<span class="status-badge dsx-309">${String(k.category.name).replace(/</g, "&lt;")}</span>` : '<span class="dsx-126">—</span>'}</td>
						<td>${statusBadge}</td>
						<td>${created}</td>
						<td>
							<div class="action-buttons">
			...
			function applyKitchenOrderChange(id, inputEl) {
				...
				if (to === from) {
					// value snapped back to its current position
				...
				renderKitchens();
				// Refocus the moved row's input so the user can keep typing.
				setTimeout(() => {
			...
			function openKitchenViewModal(id) {
				...
				// Market-owned kitchens are view-only for the main admin — hide the
				// Edit action in the view modal footer.
				const viewEditBtn = document.getElementById("kitchen-view-edit-btn");
				if (viewEditBtn) {
					viewEditBtn.style.display = (k.market && (k.market._id || k.market)) ? "none" : "";
				}
				...
				const picture = k.picture
					? `<img src="${esc(k.picture)}" alt=""  class="dsx-310">`
					: `<div class="dsx-311">${esc(initial)}</div>`;
				...
				const itemsList = items.length
					? items.map((it, i) => {
							...
							const thumb = img
								? `<img src="${esc(img)}" alt=""  class="dsx-312">`
								: `<div class="dsx-313">${esc(letter)}</div>`;
							const meta = [
							...
							return `
								<div class="dsx-315">
									<span class="dsx-316">${i + 1}.</span>
									${thumb}
									<div class="dsx-317">
										<div class="dsx-318">${esc(nm)}</div>
										${meta ? `<div class="dsx-103">${meta}</div>` : ""}
									</div>
								</div>`;
						}).join("")
					: `<div class="dsx-123">No items in this kitchen.</div>`;
				...
				body.innerHTML = `
					<div class="dsx-319">
						${picture}
						<div class="dsx-320">
							<div class="dsx-321">${esc(k.name || "—")}</div>
							<div class="dsx-322">${statusBadge}</div>
							<div class="dsx-323">
								<div class="dsx-286">Category</div><div>${k.category && k.category.name ? esc(k.category.name) : "—"}</div>
								<div class="dsx-286">Items</div><div><strong>${items.length}</strong></div>
								<div class="dsx-286">Created</div><div>${esc(created)}</div>
								<div class="dsx-286">Updated</div><div>${esc(updated)}</div>
								<div class="dsx-286">ID</div><div class="dsx-324">${esc(k._id)}</div>
							</div>
						</div>
					</div>
					<div class="dsx-325">
						<div class="dsx-326">
							...
							<span class="dsx-327">${items.length} total</span>
						</div>
						<div class="dsx-328">${itemsList}</div>
					</div>
				`;
				modal.style.display = "block";
				modal.scrollTop = 0;
			...
			async function loadKitchenSelectableProducts() {
				try {
					...
					// Keep currently-selected items even if they are no longer in stock / active.
					...
					if (listEl) listEl.innerHTML = '<div class="dsx-329">Failed to load products.</div>';
				}
			}
			...
			function renderKitchenList() {
				...
				if (!items.length) {
					...
					listEl.innerHTML = `<div class="dsx-330">${empty}</div>`;
					return;
				}
				...
				listEl.innerHTML = slice.map((p) => {
					...
					const thumb = img
						? `<img src="${img}" alt="" loading="lazy" class="dsx-331" onerror="this.outerHTML='<div class=&quot;dsx-332&quot;>${initial}</div>'" />`
						: `<div class="dsx-332">${initial}</div>`;
					const meta = [
						p.barcode ? `<span>${String(p.barcode).replace(/</g,"&lt;")}</span>` : "",
						(p.stock !== undefined && p.stock !== null) ? `<span>Stock: ${p.stock}</span>` : "",
						p.__unavailable ? '<span class="dsx-333">unavailable</span>' : ""
					].filter(Boolean).join(' <span class="dsx-314">•</span> ');
					...
					return `
						<div class="dsx-336">
							${thumb}
							<div class="dsx-317">
								<div class="dsx-337">${nameEsc}</div>
								${meta ? `<div class="dsx-103">${meta}</div>` : ""}
							</div>
							${btn}
						</div>`;
				}).join("") + (items.length > MAX_ROWS
					? `<div class="dsx-338">Showing first ${MAX_ROWS} of ${items.length}. Refine your search to see more.</div>`
					: "");
			}
			...
			function handleKitchenPictureDragLeave(e) {
				...
				const dz = document.getElementById("kitchen-picture-dropzone");
				if (!dz) return;
				// Ignore leave when moving over child elements
			...
			async function loadKitchenCategories() {
				...
				try {
					...
					if (tbody) {
						tbody.innerHTML =
							'<tr><td colspan="7" class="dsx-303">Failed to load categories.</td></tr>';
					}
			...
			function renderKitchenCategories() {
				...
				if (!kitchenCategoriesData.length) {
					tbody.innerHTML =
						'<tr><td colspan="7" class="dsx-262">No categories found. Click "Add Category" to create one.</td></tr>';
				...
				kitchenCategoriesData.forEach((c, idx) => {
					...
					// Market-owned categories are view-only for the main admin.
					const isMarketOwned = !!(c.market && (c.market._id || c.market));
					const marketName = (c.market && c.market.name) ? c.market.name : "Market";
					const marketTag = isMarketOwned
						? ` <span class="status-badge dsx-304">${marketName}</span>`
						: "";
					const actionButtons = isMarketOwned
						? `<span title="Managed by ${marketName}" class="dsx-340">View only</span>`
					...
					const imgCell = c.picture
						? `<img src="${c.picture}" alt="" onerror="this.outerHTML='<div class=&quot;dsx-306&quot;>${initial}</div>'" />`
						: `<div class="dsx-307">${initial}</div>`;
					...
					row.innerHTML = `
						<td>
							<input type="number" min="1" step="1" value="${idx + 1}"
							 class="kitchen-category-order-input dsx-308"
						...
						<td>${imgCell}</td>
						<td><strong>${c.name || "—"}</strong>${marketTag}</td>
						<td>${kCount}</td>
						<td>${statusBadge}</td>
						<td>${created}</td>
						<td>
