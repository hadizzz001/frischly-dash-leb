		...
		function closeMe() {
			...
			try { window.parent && window.parent.postMessage({ type: 'market-profile-map:close' }, '*'); } catch (_) {}
		...
		async function loadRegions() {
			...
			let initialRegions = [];
			try {
				const res = await authedFetch(API('/market-admin/profile'), { cache: 'no-store' });
		...
		async function submitRegions() {
			...
			try {
				const res = await authedFetch(API('/market-admin/profile'), {
					method: 'PUT',
					cache: 'no-store',
				...
				try { window.parent && window.parent.postMessage({ type: 'market-profile-map:saved', deliveryRegions: savedRegions }, '*'); } catch (_) {}
